...
% dfca_n = electrolyteAct(cen_bar,p.T_ref,p);
% dfca_s = electrolyteAct(ces_bar,p.T_ref,p);
% dfca_p = electrolyteAct(cep_bar,p.T_ref,p);
...
% RTaF=(p.R*p.T_ref)/(p.alph*p.Faraday);
...
% V_electrolytePolar = (2*p.R*p.T_ref)/(p.Faraday) * (1-p.t_plus)* ...
%         ( (1+dfca_n) * (log(cens) - log(ce0n)) ...
%          +(1+dfca_s) * (log(cesp) - log(cens)) ...
%          +(1+dfca_p) * (log(ce0p) - log(cesp)));
% Add 'em up!
...
% j_s = -p.i0s/p.Faraday * exp((-p.alph*p.Faraday)/(p.R*p.T_ref)*eta_s);
...
% Qdot = -cur*(V_noVCE - (Upb - Unb) + T1*(dUpbdT - dUnbdT));
% Qdot = cur^2*0.0015;
% Differential equations
T1_dot = (p.h12 * (T2-T1) + Qdot) / p.C1;
T2_dot = (p.h12 * (T1-T2) + p.h2a*(p.T_amb - T2)) / p.C2;

% T1_dot = (p.h12*(p.T_amb - T1) + Qdot) / (p.C1 + p.C2);
% T2_dot = 0;
%% Concatenate time derivatives
delta_sei_dot = 0; %ingore aging dynamics
% T1_dot = 0; %ignore temperature change
% T2_dot = 0;
