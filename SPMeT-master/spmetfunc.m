...
function otpt = spmetfunc(nls,delta_sei0)
% clear;
% clc;
% close all;
global flag
...
% DFN = csvread('Old data/DFN_10C_395_f.csv');
% ttotal = length(DFN)-15;
...
% data = zeros(length(V_spm),7);
% data(:,1) = V_spm;
% data(:,2) = SOC_n;
% data(:,3) = c_ss_n/p.c_s_n_max;
% data(:,4) = SOC_p;
% data(:,5) = c_ss_p/p.c_s_p_max;
% data(:,6) = T1;
% data(:,7) = I;
% % data(:,5) = c_ex(:,1);
% 
% csvwrite("SPM_LA92_385_f.csv",data)
% csvwrite("Vspme.csv",V)
for idxx = 1:length(t)
    if V(idxx)==0
        break
    end
...
n_Li_s_new = nls + sum(j_s(1:idxx-1))*p.L_n*p.a_s_n;
delta_Li = j_s(1:idxx-1)*p.L_n*p.a_s_n;
otpt = [n_Li_s_new mean(delta_sei(1:idxx-1)) delta_Li'];
end