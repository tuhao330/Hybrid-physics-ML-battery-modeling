...
% Molar ionic flux
jn_tot = z(2*Nnp+Nx+3 : 2*Nnp+Nx+2+Nn);
jp_tot = z(2*Nnp+Nx+2+Nn+1 : end);
...
j_s = -p.i0s/p.Faraday * exp((-p.alph*p.Faraday)/(p.R*T)*eta_s);
...
% f = [c_s_n_dot; c_s_p_dot; c_e_dot; delta_sei_dot; T_dot];
f = [c_s_n_dot; c_s_p_dot; c_e_dot; delta_sei_dot; T_dot];
