...
% current = I*21;
% % current = [current(1:1430);current(1:1230);current(2872:end)];
% % current = [current(1:2870);current(1440:2870);current(1440:end)];
% % for k = 1:length(current)
% %     if current(k)<-5.9*33.9923;
% %         current(k) = -5.9*33.9923;
% %     end
% % end
% % current = current - min(current);
% total = sum(current)/3600 % total discharge capacity
% min = min(current)/33.9923 % max chrage C-rate
% max = max(current)/33.9923 % max discharge C-rate
% current = round(current,3);
% % time = 0:1:length(current)-1;
% plot(current/33.9923)
% writematrix(current,'SC04x4_ce.csv')
% ylabel('C-rate')
% xlabel('Time')
...
t1 = 100;%1C 3460
t2 = 318-100;%1C 3460
I = [0 8*p.OneC*ones(1,t1) 8*p.OneC*ones(1,t2)];
t = 0:p.delta_t:(t1+t2);

% spmtdata = readmatrix('ThermalData\SPMT_10C.csv');
% startpt = 200;
    
% I = [ spmtdata(1:startpt-1,6); 1*p.OneC*ones(10000,1)];
...
% Average Surface Anode SOC
SOCs = zeros(NT,1);
SOCs(1) = (mean(c_ss_n(:,1)) - cn_low) / (cn_high - cn_low);
...
%     newtonStats.condJac(k+1) = stats.condJac;
    ...
    %
    SOCs(k+1) = (mean(c_ss_n(:,k+1)) - cn_low) / (cn_high - cn_low);
...
% csvwrite("DFN_02C_cyc0.csv",Volt(2:end))

%% Save data for neural network training
% datas = zeros(length(SOC),2);
% datas(:,2) = T;
% datas(:,1) = Volt;
% csvwrite("ThermalData/DFN_8C.csv",datas)
