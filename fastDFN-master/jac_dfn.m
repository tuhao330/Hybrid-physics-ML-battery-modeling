...
% deltasei derivatives
% Derivatives wrt T
dfseidT = delta_sei_dot*(-p.alph*p.Faraday)/(p.R).*eta_s*(-1/(T^2));
f_x(ind_sei,end)=dfseidT;

% Derivatives wrt delta_sei
dfseidsei = delta_sei_dot * (-aFRT) * (-p.Faraday/p.kappa_P);
f_x(ind_sei,ind_sei)=diag(dfseidsei);

% Derivatives wrt jn
dfseidjn = delta_sei_dot * (-aFRT) * (-p.Faraday).*R_tot_n;
f_z(ind_sei,ind_jn)=diag(dfseidjn);

% Derivatives wrt phi_s-
dfseidphisn = delta_sei_dot * (-aFRT);
f_z(ind_sei,ind_phi_s_n)=diag(dfseidphisn);

% Derivatives wrt phi_e-
dfseidphien = delta_sei_dot * aFRT;
f_z(ind_sei,2*Nnp+1:2*Nnp+Nn)=diag(dfseidphien);
...
% Overpotential
eta_n = phi_s_n - phi_e(1:Nn) - Unref - p.Faraday*R_tot_n.*jn;
...
df6Jn = -2/p.Faraday*i_0n.*cosh(aFRT*eta_n) * aFRT.*R_tot_n*p.Faraday - 1;
...
% jn derivatives wrt deltasei
dgjndsei = 2/p.Faraday * i_0n .* cosh(aFRT * eta_n) * aFRT * (-p.Faraday)/(p.kappa_P).*jn;
g_x(ind_jn,ind_sei)=diag(dgjndsei);
